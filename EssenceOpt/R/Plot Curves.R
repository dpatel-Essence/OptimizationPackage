...
#' @export
#' @examples
#' plot_curves(basedir,pa,date,ms_name,out_name,mcp_name,process_data_fun,cost_per_type=1)


plot_curves <- function(basedir,pa,date,ms_name,out_name,mcp_name,process_data_fun,cost_per_type=1){
  ...
  for(i in 1:length(usegs)){
    ...
    par(mar=c(12,12,8,8),family="Roboto")
    plot(act_ms,act_out,xlab="",ylab="",col=get_colors("G_Black"),las=1,cex.axis=2,lwd=3,
    ...
    lines(line_ms,line_out,lwd=3,col=get_colors("G_DRed"))
    x <- dev.off()
    ...
    png(paste(basedir,"/PAs/",pa,"/",date,"/Graphics/Cost Per Curves/",usegs[i],".png",sep=""),
        height=480*1.5*1.5,width=480*1.5*1.5)
    par(mar=c(12,12,8,8),family="Roboto")
    plot(act_ms,act_mcp,xlab="",ylab="",col=get_colors("G_Black"),las=1,cex.axis=2,lwd=3,
         font=1,cex.main=3,font.main=1,xaxt="n",yaxt="n",pch=19)
    ...
    mtext(text=full_ms_name,side=1,cex=1.9,line=7,font=1)
    lines(line_ms,line_mcp,lwd=3,col=get_colors("G_DRed"))
    x <- dev.off()
  }
}